...
///
/// Generated class from Pigeon that represents data sent in messages.
struct WriteResultMessage: Hashable {
  var outcome: String
  var persistedRecordIds: [String]? = nil
  var validationFailures: [[String: Any]]? = nil


  // swift-format-ignore: AlwaysUseLowerCamelCase
  static func fromList(_ pigeonVar_list: [Any?]) -> WriteResultMessage? {
    let outcome = pigeonVar_list[0] as! String
    let persistedRecordIds: [String]? = nilOrValue(pigeonVar_list[1])
    let validationFailures: [[String: Any]]? = nilOrValue(pigeonVar_list[2])
  ...
  static func == (lhs: WriteResultMessage, rhs: WriteResultMessage) -> Bool {
    return deepEqualsConnectKitMessages(lhs.toList(), rhs.toList())  }
  func hash(into hasher: inout Hasher) {
    deepHashConnectKitMessages(value: toList(), hasher: &hasher)
  }
...
private class ConnectKitMessagesPigeonCodecReader: FlutterStandardReader {
  override func readValue(ofType type: UInt8) -> Any? {
    ...
    case 130:
      return WriteResultMessage.fromList(self.readValue() as! [Any?])
    default:
...
private class ConnectKitMessagesPigeonCodecWriter: FlutterStandardWriter {
  override func writeValue(_ value: Any) {
    ...
    } else if let value = value as? WriteResultMessage {
      super.writeByte(130)
      super.writeValue(value.toList())
    } else {
...
protocol ConnectKitHostApi {
  ...
  func writeRecords(records: [[String: Any?]], completion: @escaping (Result<WriteResultMessage, Error>) -> Void)
}
...
class ConnectKitHostApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: ConnectKitHostApi?, messageChannelSuffix: String = "") {
    ...
    let writeRecordsChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.connect_kit.ConnectKitHostApi.writeRecords\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      writeRecordsChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let recordsArg = args[0] as! [[String: Any?]]
